   ...
   %else %do;
     ...
     %if %length(&weight.) %then %do;
         %if %sysfunc(varnum(&dsid.,&weight.)) = 0 %then %do;
            %put ERROR : STDLOGOR macro input data set &indsn. does not contain variable &weight. specified in macro parameter weight.;
            %let errcode = 1; 
            %end;
         %end;
     %if %length(&adjcov.) %then %do k = 1 %to &nadjcov.;
          %if %sysfunc(varnum(&dsid.,&&adjcov&k..)) = 0 %then %do;
            %put ERROR : STDLOOOR macro input data set &indsn. does not contain the variable &&adjcov&k.. specified in macro parameter adjcov.;
            %let errcode = 1;
            %end;
         %end;
     %if %length(&sampstrata.) %then %do k = 1 %to &nstrat.;
         %if %sysfunc(varnum(&dsid.,&&strat&k..)) = 0 %then %do;
            %put ERROR : STDLOGOR macro input data set &indsn. does not contain variable &&strat&k.. specified in macro parameter strata.;
            %let errcode = 1;
            %end;
         %end;
