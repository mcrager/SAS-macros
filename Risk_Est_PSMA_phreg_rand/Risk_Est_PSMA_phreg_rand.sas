...
Program Name           : risk_est_PSMA_phreg_rand
...
Purpose                : Macro to perform risk estimation using random effects patient-specific meta-analysis and proportional 
...
%macro risk_est_PSMA_phreg_rand(  
   ...
   /*  Analysis settings */ convergence=0.00000001,alpha=0.05,
...
| Type ($/#)     : #
| Purpose        : Convergence criterion for Newton-Raphson iteration to estimate interstudy variance.
|-----------------------------------------------------------------------------------------------
| Name           : alpha        
| Required (Y/N) : N
| Default Value  : 0.05
...
/* Check for errors in macro parameter specification */
   ...
   %if %length(&covdsn.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_phreg_rand macro parameter covdsn must be specified.;
      %abort;
   %end;

   %if %length(&common_vars.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_phreg_rand macro parameter common_vars must be specified.;
      %abort;
   %end;

   %if %length(&num_studies.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_phreg_rand macro parameter num_studies must be specified.;
      %abort;
   %end;
...
%if %length(&convergence.) = 0 %then %let convergence = 0.00000001;
...
**********************************************************************************************/
  ...
  %do k = 1 %to &num_studies.;
      ...
      %if &dsid. = 0 %then %do;
         %put ERROR :  Risk_Est_PSMA_phreg_rand macro found that specified key statistics data set &&key_stats&k... does not exist.;
         %abort;
         %end;
      %else %do;
         %do i = 1 %to &&ntotvar_&k..;
             %if %sysfunc(varnum(&dsid.,beta_&&var&i..)) = 0 %then %do;
                 %put ERROR :  Risk_Est_PSMA_phreg_rand macro key statistics data set &&key_stats&k... does not contain the required variable beta_&&var&i..;
                 %let errcode = 1;
                 %end;
             %if %sysfunc(varnum(&dsid.,ggamma_&&var&i..)) = 0 %then %do;
                 %put ERROR :  Risk_Est_PSMA_phreg_rand macro key statistics data set &&key_stats&k... does not contain the required variable ggamma_&&var&i..;
                 %let errcode = 1;
                 %end;
             %do j = 1 %to &&ntotvar_&k..;
                 %if %sysfunc(varnum(&dsid.,v_&&var&i.._&&var&j..)) = 0 %then %do;
	             %put ERROR :  Risk_Est_PSMA_phreg_rand macro key statistics data set &&key_stats&k... does not contain the required variable v_&&var&i.._&&var&j..;
	             %let errcode = 1;
                     %end;
                 %end;
	     %if %sysfunc(varnum(&dsid.,cumhaz)) = 0 %then %do;
	             %put ERROR :  Risk_Est_PSMA_phreg_rand macro key statistics data set &&key_stats&k... does not contain the required variable cumhaz.;
	             %let errcode = 1;
		     %end;	     
	     %if %sysfunc(varnum(&dsid.,stdcumhaz)) = 0 %then %do;
	             %put ERROR :  Risk_Est_PSMA_phreg_rand macro key statistics data set &&key_stats&k... does not contain the required variable stdcumhaz.;
	             %let errcode = 1;
		     %end;
             %if %length(&strata.) %then %do j = 1 %to &nstrata.;
                     %if %sysfunc(varnum(&dsid.,&&strat&j..)) = 0 %then %do;
                         %put ERROR : GHI Note: Risk_Est_PSMA_phreg_rand macro found that covariate data set &&key_stats&k... does not contain the required variable &&strat&j.. specified in macro parameter strata.;
                         %let errcode = 1;
                         %end;
                     %end; 
             %end;
   ...
   %if &dsid. = 0 %then %do;
       %put ERROR :  Risk_Est_PSMA_phreg_rand macro found that specified covariate data set &covdsn. does not exist.;
       %abort;
       %end;
   %else %do;
       %do j = 1 %to &ntotvar.;
           %if %sysfunc(varnum(&dsid.,&&var&j..)) = 0 %then %do;
               %put ERROR :  Risk_Est_PSMA_phreg_rand macro found that covariate data set &covdsn. does not contain the required variable &&var&j..;
               %let errcode = 1;
               %end;
            %end;
       %if %length(&strata.) %then %do j = 1 %to &nstrata.;
           %if %sysfunc(varnum(&dsid.,&&strat&j..)) = 0 %then %do;
              %put ERROR :  Risk_Est_PSMA_phreg_rand macro found that covariate data set &covdsn. does not contain the required variable &&strat&j.. specified in macro parameter strata.;
              %let errcode = 1;
              %end;
           %end;        
       %end;
...
%end;
    ...
    array W(&num_studies.) W1-W&num_studies.;
    array qqest0(&num_studies.) rho01-rho0&num_studies.;
    array qqsigma20(&num_studies.) variance01-variance0&num_studies.;
...
/***********************************************************************************************/
/**  Log cumulative hazard estimate and its variance with special population indicators set   **/
/**  to 0.                                                                                    **/
/***********************************************************************************************/       
...
%do k = 1 %to &num_studies.;
    ...
    betatx0&k. = 0;
    do i = 1 to &ntotvar.;
       betatx0&k. = betatx0&k. + beta_&k.(i) * z(i);
       end; 
       
    rho0&k. = betatx0&k. + log(cumhaz_&k.); 
...
/*********************************************************************************************/
/**  Find the estimate of inter-study variability tau2 using Newton-Raphson iteration.  Set **/
/**  to 0 if the solution is negative.                                                      **/
/*********************************************************************************************/

**  Initialize values;
     tau2 = 0;
     step = 10 * &convergence; *  Arbitrary value > convergence criterion;
     F = 0;
     flag = 0;  *  This flag will be set to 1 if the objective function F is negative for tau2 = 0
                *  on the first iteration (tau2 = 0 ) in which case no further iterations are required;

     do i = 1 to 10000 while (flag = 0 and abs(step) > &convergence);

**  Compute weights;
	 Wsum = 0;
	 do k = 1 to &num_studies.;
	      W(k) = 1 / (tau2 + qqsigma20(k));
	      Wsum = Wsum + W(k);
	      end;
...
**  Compute weighted average estimate;
         qqestW = 0;
	 do k = 1 to &num_studies.;
	       qqestW = qqestW + (W(k)/Wsum) * qqest0(k);
	       end;

**  Compute objective function;
         F = 0;
	 do k = 1 to &num_studies.;
	      F = F + W(k) * (qqest0(k) - qqestW)**2;
	      end;
         F = F - (&num_studies.-1);

	 if i = 1 and F <= 0 then do;
            tau2 = 0;
            flag = 1;
            end;
	    ...
	    densum = 0;
            ...
            do k = 1 to &num_studies.;
	       numsum = numsum + W(k) * ((qqest0(k)-qqestW)**2);
               densum = densum + ((W(k))**2) * ((qqest0(k)-qqestW)**2);
               end;
         
            numsum = numsum - (&num_studies.-1);
...
/****************************************************************************************/
/**  Algorithm converged.  Compute meta-analysis estimate and its estimated variance   **/
/****************************************************************************************/
...
** Variance of tau2;
        ...
        do k = 1 to &num_studies.;
             Wsumk = Wsum - W(k);
             xpsum_num = xpsum_num + ((W(k) * Wsumk / Wsum)**2) * ((qqest0(k) - qqestW)**2) * (qqsigma20(k) + tau2);
             xpsum_den = xpsum_den + (W(k)**2) * ((qqest0(k) - qqestW)**2);
             littlew_sum2 = littlew_sum2 + (W(k)/Wsum)**2;
             end;
         
        var_tau2 = 4 * xpsum_num / (xpsum_den**2); 

**  First and second derivatives of meta-analysis estimate with respect to tau2;
       ...
       do k = 1 to &num_studies.;
            Wsum2 = Wsum2 + W(k)**2;
            xpsum2 = xpsum2 + (W(k)**2) * (qqest0(k)-qqestW);
            xpsum3 = xpsum3 + (W(k)**3) * (qqest0(k)-qqestW);
            end;
                  
       D1 = xpsum2 / Wsum;
       D2 =  - 2 * (xpsum2 * Wsum2 - xpsum3 * Wsum)  / (Wsum**2);
...
/***********************************************************************************************/    
    ...
    variance&k. = sum + (stdcumhaz_&k. / cumhaz_&k.)**2 + tau2; 
    
%end;

/***********************************************************************************************/
/**  Random effects meta-analysis estimate of log cumulative hazard and its variance.         **/   
/***********************************************************************************************/
...
%end;
    ...
    variance_rho = variance_rho + var_tau2 * (D1**2) + 0.5 * (var_tau2**2) * (D2**2);
