...
Purpose                : Compute the standardized absolute log odds ratio for treatment interaction in a multivariate logistic regression  
...
%macro STDLOGOR_INT(
        /* Input Specification */  indsn=,byvar=,response=,vars=,tmt=,weight=,sampstrata=,adjcov=,
...
|                  that interact with treatment and for which the standardized absolute log odds ratio will be computed.
|-----------------------------------------------------------------------------------------------
| Name           : tmt
| Required (Y/N) : Y
| Default Value  :
...
|                  model but which do not interact with treatment and are not included in the interaction 
|                  standardized odds ratio computation.
|-----------------------------------------------------------------------------------------------
| Name           : sampstrata   
| Required (Y/N) : N
| Default Value  : 
   ...
   %else %do;
     ...
     %if %length(&tmt.) %then %do;
         %if %sysfunc(varnum(&dsid.,&tmt.)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain variable &tmt. specified in macro parameter tmt.;
            %let errcode = 1; 
            %end;
         %end;
...
proc logistic data=&t..indata NAMELEN=30;
     by &byvar.;;
     model &response. = %do i = 1 %to &nvar.; &&var&i.. %end; 
                       &tmt. %do i = 1 %to &nvar.; &&var&i.._tmt %end;
                       &adjcov. / covb;   
run;
...
%do i = 1 %to &nvar.;
      if upcase(Parameter) = upcase("&&var&i.._tmt") then row = &i;
%end;
