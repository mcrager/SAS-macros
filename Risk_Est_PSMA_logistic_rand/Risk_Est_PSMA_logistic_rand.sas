...
Program Name           : risk_est_PSMA_logistic_rand
...
%macro risk_est_PSMA_logistic_rand(
...
/* Check for errors in macro parameter specification */
   ...
   %if %length(&covdsn.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_logistic_rand macro parameter covdsn must be specified.;
      %abort;
   %end;

   %if %length(&common_vars.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_logistic_rand macro parameter common_vars must be specified.;
      %abort;
   %end;

   %if %length(&num_studies.)=0 %then %do;
      %put ERROR :  Risk_Est_PSMA_logistic_rand macro parameter num_studies must be specified.;
      %abort;
   %end;
...
**********************************************************************************************/
  ...
  %do k = 1 %to &num_studies.;
      ...
      %if &dsid. = 0 %then %do;
         %put ERROR :  Risk_Est_PSMA_logistic_rand macro found that specified key statistics data set &&key_stats&k... does not exist.;
         %abort;
         %end;
      %else %do;
         %do i = 1 %to &&ntotvar_&k..;
             %if %sysfunc(varnum(&dsid.,beta_&&var&i..)) = 0 %then %do;
                 %put ERROR :  Risk_Est_PSMA_logistic_rand macro key statistics data set &&key_stats&k... does not contain the required variable beta_&&var&i..;
                 %let errcode = 1;
                 %end;

             %do j = 1 %to &&ntotvar_&k..;
                 %if %sysfunc(varnum(&dsid.,v_&&var&i.._&&var&j..)) = 0 %then %do;
	             %put ERROR :  Risk_Est_PSMA_logistic_rand macro key statistics data set &&key_stats&k... does not contain the required variable v_&&var&i.._&&var&j..;
	             %let errcode = 1;
                     %end;
                 %end;
             %end;
   ...
   %if &dsid. = 0 %then %do;
       %put ERROR :  Risk_Est_PSMA_logistic_rand macro found that specified covariate data set &covdsn. does not exist.;
       %abort;
       %end;
   %else %do k = 1 %to &ntotvar.;
       %if %UPCASE(&&var&k..) ne INTERCEPT and %sysfunc(varnum(&dsid.,&&var&k..)) = 0 %then %do;
           %put ERROR :  Risk_Est_PSMA_logistic_rand macro found that covariate data set &covdsn. does not contain the required variable &&var&k..;
           %let errcode = 1;
           %end;
       %end;
   %let rc = %sysfunc(close(&dsid.));
...
/***********************************************************************************************/
/**  Random effects meta-analysis estimate of log odds and its variance.                      **/
/***********************************************************************************************/
...
%mend risk_est_PSMA_logistic_rand;     
