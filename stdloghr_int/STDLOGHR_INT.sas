...
Purpose                : Compute the standardized absolute log hazard ratio for treatment interaction from a multivariate Cox 
                         proportional hazards regression model,and associated variability estimates, test statistics and 
...
Output Datasets/Views  : Specified SAS data set outdsn containing the standardized absolute log hazard ratio and its standard error.                         
...
%macro STDLOGHR_INT(
        /* Input Specification */  indsn=,byvar=,vars=,tmt=,time=,censor=,censorlist=0,weight=,adjcov=,strata=,
...
| Default Value  :
| Type ($/#)     : $ or #
| Purpose        : List of input data set variables containing the Cox model covariates that interact 
|                  with treatment in the model.
|-----------------------------------------------------------------------------------------------
| Name           : tmt
| Required (Y/N) : Y
| Default Value  :
| Type ($/#)     : #
| Purpose        : Input data set treatment indicator variable.  
|-----------------------------------------------------------------------------------------------
| Name           : time
| Required (Y/N) : Y
| Default Value  : 
...
|                  model but do not interact with treatment.
|-----------------------------------------------------------------------------------------------
| Name           : strata   
| Required (Y/N) : N
| Default Value  : 
...
|                  and stratification variables, the partial standardized log hazard ratio for interaction will be calculated
...
|                  that the absolute standardized hazard ratio for interaction is less than &intvl_hyp.                      
|-----------------------------------------------------------------------------------------------
| Name           : outdsn  
| Required (Y/N) : Y
| Default Value  : (at temporary library)
...
/* Check for errors in macro parameter specification */
   ...
   %if %length(&indsn.)=0 %then %do;
      %put ERROR : STDLOGHR_INT macro parameter indsn must be specified.;
      %abort;
   %end;

   %if %length(&vars.)=0 %then %do;
      %put ERROR : STDLOGHR_INT macro parameter vars must be specified.;
      %abort;
   %end;
   
   %if %length(&tmt.)=0 %then %do;
      %put ERROR : STDLOGHR_INT macro parameter tmt must be specified.;
      %abort;
   %end;

   %if %length(&time.)=0 %then %do;
      %put ERROR : STDLOGHR_INT macro parameter time must be specified.;
      %abort;
   %end;

   %if %length(&censor.)=0 %then %do;
      %put ERROR : STDLOGHR_INT macro parameter censor must be specified.;
      %abort;
   %end;

   %if %length(&outdsn.)=0 %then %do;
      %put ERROR :  STDLOGHR_INT macro parameter outdsn must be specified.;
      %abort;
   %end;

   %if %length(&alpha.) %then %do;
   %if %sysevalf(&alpha. <= 0) or %sysevalf(&alpha. >= 1) %then %do;
      %put ERROR : STDLOGHR_INT macro parameter alpha must be >0 and <1.;
      %abort;
   %end;
   %end; 
   ...
   %if %length(&var_combo_indsn.) > 0 and %length(&contribution_prefix.) = 0 %then %do;
       %put  ERROR :  STDLOGHR_INT macro found parameter var_combo_indsn set but parameter contribution_prefix not set.;
       %abort;
       %end;   
...
%if &doublespec_flag. > 0 %then %do;
    %put ERROR : STDLOGHR_INT macro found variable &&var&doublespec_flag.. listed in both parameter vars and parameter adjcov.  These two parameters must have distinct lists of variables.;
    %abort;
    %end;
...
%do i = 1 %to &npartial.;
   ...
   %else %if %index(&strata.,&pvar.) > 0 %then %do;
      ...
      %end;
   %else %do;
      %put ERROR : STDLOGHR_INT macro parameter found specified partial variable &pvar..;
...
**********************************************************************************************/
  ...
  %if &dsid. = 0 %then %do;
     %put ERROR : STDLOGHR_INT macro found that specified input data set &indsn. does not exist.;
     %abort;
     %end;
   %else %do;
     ...
     %if %sysfunc(varnum(&dsid.,&censor.)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain the variable &censor. specified in macro parameter censor.;
            %let errcode = 1;
            %end; 
     %do k = 1 %to &nvar.;
          %if %sysfunc(varnum(&dsid.,&&var&k..)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain the variable &&var&k.. specified in parameter vars.;
            %let errcode = 1;
            %end;
         %end;
     %if %length(&weight.) %then %do;
         %if %sysfunc(varnum(&dsid.,&weight.)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain variable &weight. specified in macro parameter weight.;
            %let errcode = 1; 
            %end;
         %end;
     %if %length(&adjcov.) %then %do k = 1 %to &nadjcov.;
          %if %sysfunc(varnum(&dsid.,&&adjcov&k..)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain the variable &&adjcov&k.. specified in macro parameter adjcov.;
            %let errcode = 1;
            %end;
         %end;
     %if %length(&strata.) %then %do k = 1 %to &nstrat.;
         %if %sysfunc(varnum(&dsid.,&&strat&k..)) = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro input data set &indsn. does not contain variable &&strat&k.. specified in macro parameter strata.;
            %let errcode = 1;
            %end;
         %end;
     ...
     %if %length(&var_combo_indsn.) %then %do;
        ...
        %if &dsid. = 0 %then %do;
            %put ERROR : STDLOGHR_INT macro variable found that specified combination indicator data set &var_combo_indsn. does not exist.;
            %abort;
            %end;
        %else %do k = 1 %to &nvar.;
            %if %sysfunc(varnum(&dsid.,ind_&&var&k..)) = 0 %then %do;
                %put ERROR : STDLOGHR_INT macro variable combination indicator data set &var_combo_indsn. does not contain the required variable ind_&&var&k..;
                %let errcode = 1;
                %end;
            %end;
        %let rc = %sysfunc(close(&dsid.));
...
by &byvar.;
     model &time.*&censor.(&censorlist.) = %do i = 1 %to &nvar.; &&var&i.. %end; &tmt.
                                           %do i = 1 %to &nvar.; &&var&i.._tmt %end;
...
%end;                           
                      %else %do;
/**********************************************************************************************
**  Case 3: Cohort sampling design with analysis using Lin and Wei covariance sandwich method.**
**********************************************************************************************/
...
data &t..v;
      array covariate(&nvar.) %do i = 1 %to &nvar.; &&var&i.._tmt %end;; 
...
%do i = 1 %to &nvar.;
      if upcase(_NAME_) = upcase("&&var&i.._tmt") then row = &i;
%end;
